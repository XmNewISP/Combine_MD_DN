...
#必须和MdNet的参数保持一致
MD_NET_RECEPTIVE_FIELD = 18
MD_NET_STRIDE = 8
MD_NET_THRESHOULD = 0.05
...
def do_movedetect(model,oldframe,curframe):
    ...
    if use_cuda:
        oldT = oldT.cuda()
        newT = newT.cuda()
    output = model(oldT,newT,MD_NET_RECEPTIVE_FIELD,MD_NET_STRIDE,MD_NET_THRESHOULD) #误差门限(/256)
